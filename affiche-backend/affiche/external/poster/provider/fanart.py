...
class FanartClient(ExternalProvider):
    ...
    @staticmethod
    def _image(entry: dict) -> PosterImage:
        lang = entry.get("lang") or None
        if lang == "00":
            return PosterImage(entry["url"], textless=True)
        return PosterImage(entry["url"], language=lang, textless=False if lang else None)
    ...
    def _fetch_all_season_poster(self, tvdb_id: int, season_number: int) -> List[str]:
        try:
            ...
            for poster in season_posters:
                if poster.get("season") == str(season_number) and poster.get("url"):
                    posters.append(self._image(poster))
