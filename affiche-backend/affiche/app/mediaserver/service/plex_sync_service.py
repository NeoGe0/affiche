...
class PlexSynchronisationService:
    BATCH_SIZE = 100
    ...
    def _connector(self, media_server: MediaServer) -> PlexService:
        connector = self._connector_factory.get(media_server.id)
        if not isinstance(connector, PlexService):
            raise ValueError(f"Media server {media_server.id} is not a Plex server")
        return connector

    def sync_plex_libraries(self,
                            media_server: MediaServer,
                            cancel_check=None):
        logger.info("Starting Plex library sync")
        plex_service = self._connector(media_server)
        ...
        for library in libraries:
            if cancel_check and cancel_check():
                logger.info("Plex library sync cancelled")
                break
            if library.enabled:
                try:
                    self._sync_library(plex_service, library, cancel_check=cancel_check)
                except Exception:
        ...
        logger.info("Plex library sync completed")
    ...
    def sync_plex_library(self,
        ...
        logger.info("Starting Plex library %s %s sync", library_id,
    ...
    def sync_plex_item(self, media_server: MediaServer, library_id: int, item_id: int):
        library = self.library_service.get_library(media_server.id, library_id)
        db_item = self.library_service.get_library_item(media_server.id, library_id, item_id)

        plex_service = self._connector(media_server)
        fetched = plex_service.get_library_item(db_item.external_id)
        if not fetched:
            logger.info("[sync] item %s (%s) no longer on Plex", item_id, db_item.title)
            return None
        ...
        synced_id = self.library_service.merge_readded_twin(
            ...
            is_gone=lambda external_id: plex_service.get_library_item(external_id) is None,
        ) or item_id
        ...
        if fetched.type == 'show':
            self._sync_seasons(plex_service, library, [fetched])
            if self._episodes_tracked(library):
                self._sync_episodes(plex_service, library, [fetched])
    ...
    def _fetch_items(self, plex_service: PlexService, library: Library, incremental: bool):
        if not incremental:
    ...
    def _sync_library(self,
        ...
        library_items, incremental = self._fetch_items(plex_service, library, incremental)
        ...
        if shows_to_sync and not (cancel_check and cancel_check()):
            self._sync_seasons(plex_service, library, shows_to_sync)
            if self._episodes_tracked(library) and not (cancel_check and cancel_check()):
                self._sync_episodes(plex_service, library, shows_to_sync)

        if incremental or (cancel_check and cancel_check()):
            return

        if self._collections_tracked(library):
            self._sync_collections(plex_service, library, seen_at)
    ...
    def _sync_collections(self, plex_service: PlexService, library: Library, seen_at):
        try:
            collections = plex_service.get_collections(library.external_id)
        except Exception:
            logger.exception("Failed to fetch collections for library '%s'", library.name)
            return

        self.collection_service.sync_collections(
            library.id, [c.model_dump() | {'external_id': c.id} for c in collections], seen_at)

    def _sync_seasons(self, plex_service: PlexService, library: Library, shows: list):
        ...
        for show in shows:
            try:
                ...
                plex_seasons = plex_service.get_show_seasons(str(show.id), int(library.external_id))

                rekeyed = self.season_service.adopt_readded_seasons(
                    library.id, internal_show_id,
                    {season.season_number: str(season.id) for season in plex_seasons})
                if rekeyed:
                    self.episode_service.delete_episodes_of_seasons(rekeyed)

                for plex_season in plex_seasons:
                    season = LibrarySeason(
                        external_id=str(plex_season.id),
                        show_id=internal_show_id,
                        library_id=library.id,
                        season_number=plex_season.season_number,
                        title=plex_season.title,
                        added_at=plex_season.added_at,
                        updated_at=plex_season.updated_at,
                        imdb_id=plex_season.imdb_id,
                        tmdb_id=plex_season.tmdb_id,
                        tvdb_id=plex_season.tvdb_id,
                        poster_url=plex_season.poster_url,
                    )
    ...
    def _sync_episodes(self, plex_service: PlexService, library: Library, shows: list):
        ...
        for show in shows:
            try:
                ...
                plex_episodes = plex_service.get_show_episodes(str(show.id), int(library.external_id))
